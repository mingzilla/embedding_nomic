### HuggingFace (HF) models, GGUF models, ONNX models
...
- The embedding created by LlamaSharp seems to be different from what's created by the HuggingFace model, I have not verified if it's compatible with the HF version
 
 
---
 
I would love to get it to work in C#, but in this case it doesn't seem to be an easy task.
 
---
 
I'm in the process of uploading the step2 duckdb file (step2). Here's an explanation of the files:
 

The 3 steps result in the step 3 parquet file, which has embedding for (CompanyName + (Company_Description -> if_not_null)) -> max_length_1024.
 
### step1_output__companies_with_description__ordered.7z
 
- This file is created with a table that has such a column: (CompanyName + (Company_Description -> if_not_null)) -> max_length_1024
- The data is ordered by CompanyNumber
 
### step2_output__all-minilm-l6-v2__company_name_with_description.7z
 
- This file adds a column, which stores the embedding
- This is produced by the python procedure
- It includes both active and inactive companies
 
### step3_output__all-minilm-l6-v2__company_name_with_description__active.parquet
 
- This is the step 2 file converted into parquet
- It only includes active companies